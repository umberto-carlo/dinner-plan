...
public class DinnerController {
    ...
    public String dashboard(Model model, @AuthenticationPrincipal UserDetails userDetails) {
        model.addAttribute("events", dinnerService.getAllEvents());
        if (userDetails != null) {
    ...
    public String createEvent(@RequestParam String title, @RequestParam String description,
            @RequestParam String deadline, @AuthenticationPrincipal UserDetails userDetails) {
        // deadline parsed from string. assuming HTML5 datetime-local input, format:
        // yyyy-MM-ddTHH:mm
        LocalDateTime dt = LocalDateTime.parse(deadline);
        dinnerService.createEvent(title, description, dt, userDetails.getUsername());
        return "redirect:/";
    }
    ...
    public String eventDetails(@PathVariable Long id, Model model, @AuthenticationPrincipal UserDetails userDetails) {
        ...
        model.addAttribute("recentProposals", dinnerService.getRecentUniqueProposals());

        List<Proposal> sortedProposals = new ArrayList<>(event.getProposals());
        sortedProposals.sort(Comparator.comparingInt((Proposal p) -> p.getVotes().size()).reversed());
