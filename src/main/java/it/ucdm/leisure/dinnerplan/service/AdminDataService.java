...
public class AdminDataService {
    ...
    public byte[] exportData() throws IOException {
        ...
        try (ZipOutputStream zos = new ZipOutputStream(baos)) {
            ...
            writeEntry(zos, "users.json",
                    userRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
            writeEntry(zos, "events.json",
                    dinnerEventRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
            writeEntry(zos, "proposals.json",
                    proposalRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
            writeEntry(zos, "proposal_dates.json",
                    proposalDateRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
            writeEntry(zos, "ratings.json",
                    proposalRatingRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
            writeEntry(zos, "votes.json",
                    voteRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
            writeEntry(zos, "messages.json",
                    messageRepository.findAll().stream().map(this::toBackupDTO).collect(Collectors.toList()));
        }
    ...
    // Mappers
    private UserBackupDTO toBackupDTO(User entity) {
    ...
    private DinnerEventBackupDTO toBackupDTO(DinnerEvent entity) {
        return DinnerEventBackupDTO.builder()
                ...
                .selectedProposalDateId(
                        entity.getSelectedProposalDate() != null ? entity.getSelectedProposalDate().getId() : null)
                .status(entity.getStatus())
                .participantIds(entity.getParticipants().stream().map(User::getId).collect(Collectors.toList()))
                .build();
    }

    private ProposalBackupDTO toBackupDTO(Proposal entity) {
        return ProposalBackupDTO.builder()
                .id(entity.getId())
                .dinnerEventIds(entity.getDinnerEvents().stream().map(DinnerEvent::getId).collect(Collectors.toList()))
    ...
    private ProposalDateBackupDTO toBackupDTO(ProposalDate entity) {
        return ProposalDateBackupDTO.builder()
                .id(entity.getId())
                .date(entity.getDate())
                .proposalId(entity.getProposal().getId())
                .dinnerEventId(entity.getDinnerEvent() != null ? entity.getDinnerEvent().getId() : null)
                .build();
    }

    private ProposalRatingBackupDTO toBackupDTO(ProposalRating entity) {
    ...
    private VoteBackupDTO toBackupDTO(Vote entity) {
        return VoteBackupDTO.builder()
                .id(entity.getId())
                .userId(entity.getUser().getId())
                .proposalDateId(entity.getProposalDate().getId())
                .build();
    }

    private DinnerEventMessageBackupDTO toBackupDTO(DinnerEventMessage entity) {
